...
from .init_halo_model import a_sf, all_param_defaults, cM_relation, cosmo, hmd, z_lens
...
def test_kSZ_frequencies():
    ...
    ρ_3D = lambda r: prof_gas.real(cosmo, r, 10 ** param_dict["log10_M"], a_sf)

    T_kSZ_f150 = glasz.kSZ.create_T_kSZ_profile(
        theta, z_lens, ρ_3D, frequency="f150", cosmo=cosmo
    )

    T_kSZ_f090 = glasz.kSZ.create_T_kSZ_profile(
        theta, z_lens, ρ_3D, frequency="f090", cosmo=cosmo
    )
...
def test_kSZ_convolution():
    ...
    T_kSZ_f150_hankel = glasz.kSZ.create_T_kSZ_profile(
        theta, z_lens, ρ_3D, frequency="f150", cosmo=cosmo, method="hankel"
    )

    T_kSZ_f150_bf = glasz.kSZ.create_T_kSZ_profile(
        theta, z_lens, ρ_3D, frequency="f150", cosmo=cosmo, method="brute_force"
    )
    ...
    T_kSZ_f090_hankel = glasz.kSZ.create_T_kSZ_profile(
        theta, z_lens, ρ_3D, frequency="f090", cosmo=cosmo, method="hankel"
    )

    T_kSZ_f090_bf = glasz.kSZ.create_T_kSZ_profile(
        theta, z_lens, ρ_3D, frequency="f090", cosmo=cosmo, method="brute_force"
    )
