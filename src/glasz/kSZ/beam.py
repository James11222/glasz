...
def beam_transform_to_profile(B_ell:NDArray[Numeric], 
                              theta:NDArray[Numeric], 
...
def generate_beam_profile(frequency:str, space:str="real") -> Callable[[NDArray[Numeric]], NDArray[Numeric]]:
    """
    ...
    if frequency == "f150":
        ...
        raise ValueError("Frequency must be either f150 or f090")
    ...
    if space == "real":
        ...
        def profile_func(θ: NDArray[Numeric]) -> NDArray[Numeric]:
            return np.interp(θ, theta, profile / integral, period=np.pi)
        ...
        raise ValueError("Space must be either real or harmonic")
